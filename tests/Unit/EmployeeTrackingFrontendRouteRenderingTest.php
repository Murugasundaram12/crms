...
class EmployeeTrackingFrontendRouteRenderingTest extends TestCase
{
    ...
    public function test_actual_route_uses_ordered_backend_segments_and_road_routing_when_available(): void
    {
        ...
        $this->assertStringContainsString('timelineConfig.snapRouteUrl', $this->source);
        $this->assertStringContainsString('timelineConfig.roadRouteEnabled', $this->source);
        $this->assertStringContainsString("data.snapped", $this->source);
    ...
    public function test_road_route_chunking_preserves_boundaries_and_falls_back_to_raw_gps(): void
    {
        ...
        $this->assertStringContainsString("'SNAP_FAILED'", $this->source);
        $this->assertStringContainsString("'NETWORK_ERROR'", $this->source);
    }
    ...
    public function test_route_markers_are_mapped_to_snapped_route_preserving_raw_coordinates_and_monotonic_order(): void
    {
        $this->assertStringContainsString('function findNearestSnappedPoint(rawPoint, snappedPoints, minSearchIndex = 0)', $this->source);
        $this->assertStringContainsString('function mapMarkersToSnappedRoute(routeMarkers, snappedSegments, movementPaths = [])', $this->source);
        $this->assertStringContainsString('function mapAttendanceMarkersToSnappedRoute(attendanceMarkers, snappedSegments, movementPaths = [])', $this->source);
        $this->assertStringContainsString('const mappedRouteMarkers = mapMarkersToSnappedRoute(routeMarkers, snappedSegments, movementPaths);', $this->source);
        $this->assertStringContainsString('const mappedAttendanceMarkers = mapAttendanceMarkersToSnappedRoute(attendanceMarkers, snappedSegments, movementPaths);', $this->source);
        $this->assertStringContainsString('rawLat,', $this->source);
        $this->assertStringContainsString('rawLng,', $this->source);
    ...
    {
        $this->assertStringContainsString('fitTimelineMapToPoints(boundsPoints.concat(mappedRouteMarkers).concat(mappedAttendanceMarkers));', $this->source);
