    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-body bg-light-subtle">
            <div class="d-flex align-items-center justify-content-between">
                ...
                <span class="badge badge-soft-info p-2"><i class="ti ti-info-circle me-1"></i>Labour salary payment will be debited directly from your wallet balance.</span>
            </div>
        </div>
    </div>
            ...
            <form action="{{ route('labour-salaries.store') }}" method="POST">
                ...
                <div class="row g-3">
                    <div class="col-12 col-md-6">
                        <label class="form-label required">Labour (Search by Name / Phone)</label>
                        <select name="labour_id" id="labour_id" class="form-select @error('labour_id') is-invalid @enderror" required>
                            ...
                            @foreach ($labours as $labour)
                                ...
                                <option value="{{ $labour->id }}" data-salary="{{ $labour->salary }}" @selected(old('labour_id') == $labour->id)>
                                    {{ $labour->name }} {{ $phone ? '– ' . $phone : '' }} (Wage: Rs {{ number_format((float) $labour->salary, 2) }})
                                </option>
                            @endforeach
                        </select>
                        @error('labour_id')
                    ...
                    <div class="col-12 col-md-3">
                        ...
                        <input type="date" name="salary_period_start" class="form-control @error('salary_period_start') is-invalid @enderror" value="{{ old('salary_period_start') }}">
                    ...
                    <div class="col-12 col-md-3">
                        ...
                        <input type="date" name="salary_period_end" class="form-control @error('salary_period_end') is-invalid @enderror" value="{{ old('salary_period_end') }}">
                    ...
                    <div class="col-12 mt-4">
                        <button type="submit" class="btn btn-primary me-2">Save Labour Salary & Debit Wallet</button>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                labourSelect.addEventListener('change', function() {
                    ...
                    if (opt && opt.dataset.salary) {
                        const defaultSalary = parseFloat(opt.dataset.salary) || 0;
                        totalInput.value = defaultSalary.toFixed(2);
                        paidInput.value = defaultSalary.toFixed(2);
                    }
                    calculate();
                });
