...
@section('content')
<div class="d-flex justify-content-between align-items-center mb-3"><h4 class="mb-0">Vendor Unpaid History</h4></div>
...
<div class="table-responsive"><table class="table table-bordered"><thead><tr><th>Vendor</th><th>Paid</th><th>Unpaid</th><th>Date</th></tr></thead><tbody>@forelse($transactions as $tx)<tr><td>{{ $tx->vendor?->name ?? '-' }}</td><td>{{ number_format((float)$tx->paid_amount,2) }}</td><td>{{ number_format((float)$tx->unpaid_amount,2) }}</td><td>{{ $tx->current_date }}</td></tr>@empty<tr><td colspan="4" class="text-center">No records</td></tr>@endforelse</tbody></table></div>
{{ $transactions->links() }}
@endsection
