...
@section('content')
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-body">
            <form action="{{ route('employee-salaries.store') }}" method="POST" id="salaryForm">
                @csrf

                <!-- Hidden Breakdown Fields -->
                <input type="hidden" name="monthly_salary" id="field_monthly_salary" value="{{ old('monthly_salary', 0) }}">
                <input type="hidden" name="working_days" id="field_working_days" value="{{ old('working_days', 0) }}">
                <input type="hidden" name="present_days" id="field_present_days" value="{{ old('present_days', 0) }}">
                <input type="hidden" name="half_days" id="field_half_days" value="{{ old('half_days', 0) }}">
                <input type="hidden" name="paid_leave_days" id="field_paid_leave_days" value="{{ old('paid_leave_days', 0) }}">
                <input type="hidden" name="unpaid_leave_days" id="field_unpaid_leave_days" value="{{ old('unpaid_leave_days', 0) }}">
                <input type="hidden" name="absent_days" id="field_absent_days" value="{{ old('absent_days', 0) }}">
                <input type="hidden" name="per_day_salary" id="field_per_day_salary" value="{{ old('per_day_salary', 0) }}">
                <input type="hidden" name="gross_salary" id="field_gross_salary" value="{{ old('gross_salary', 0) }}">
                <input type="hidden" name="half_day_deduction" id="field_half_day_deduction" value="{{ old('half_day_deduction', 0) }}">
                <input type="hidden" name="unpaid_leave_deduction" id="field_unpaid_leave_deduction" value="{{ old('unpaid_leave_deduction', 0) }}">
                <input type="hidden" name="absent_deduction" id="field_absent_deduction" value="{{ old('absent_deduction', 0) }}">
                <input type="hidden" name="attendance_deduction" id="field_attendance_deduction" value="{{ old('attendance_deduction', 0) }}">
                <input type="hidden" name="net_salary" id="field_net_salary" value="{{ old('net_salary', 0) }}">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12 col-md-4">
                        ...
                        <input type="text" name="salary_period" id="salary_period_input" class="form-control @error('salary_period') is-invalid @enderror" value="{{ old('salary_period', now()->format('F Y')) }}" placeholder="e.g. July 2026" required>
                        @error('salary_period')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12 col-md-3 d-flex align-items-end">
                        <button type="button" id="btn_calculate_payroll" class="btn btn-outline-primary w-100">
                            <i class="ti ti-calculator me-1"></i>Calculate Payroll
                        </button>
                    </div>

                    <!-- Interactive Payroll Breakdown Card -->
                    <div class="col-12" id="payroll_summary_container" style="display: none;">
                        <div class="card border border-primary-subtle bg-primary-subtle bg-opacity-10 shadow-none">
                            <div class="card-body">
                                <div class="d-flex align-items-center justify-content-between mb-3 border-bottom pb-2">
                                    ...
                                    <span class="badge bg-primary" id="badge_period_text">-</span>
                                </div>
                                <div class="row g-3 text-dark">
                                    <div class="col-6 col-md-2">
                                        <small class="text-muted d-block">Working Days</small>
                                        <strong id="disp_working_days">0</strong>
                                    </div>
                                    <div class="col-6 col-md-2">
                                        <small class="text-muted d-block">Present Days</small>
                                        <strong class="text-success" id="disp_present_days">0</strong>
                                    </div>
                                    <div class="col-6 col-md-2">
                                        <small class="text-muted d-block">Half Days</small>
                                        <strong class="text-warning" id="disp_half_days">0</strong>
                                    </div>
                                    <div class="col-6 col-md-2">
                                        <small class="text-muted d-block">Paid Leave</small>
                                        <strong class="text-info" id="disp_paid_leave">0</strong>
                                    </div>
                                    <div class="col-6 col-md-2">
                                        <small class="text-muted d-block">Unpaid Leave</small>
                                        <strong class="text-danger" id="disp_unpaid_leave">0</strong>
                                    </div>
                                    <div class="col-6 col-md-2">
                                        <small class="text-muted d-block">Absent Days</small>
                                        <strong class="text-danger" id="disp_absent_days">0</strong>
                                    </div>
                                </div>
                                <hr class="my-3">
                                <div class="row g-3">
                                    <div class="col-12 col-md-3">
                                        ...
                                        <span class="fs-6 fw-semibold" id="disp_monthly_salary">Rs 0.00</span>
                                    </div>
                                    <div class="col-12 col-md-3">
                                        <small class="text-muted d-block">Per Day Salary</small>
                                        <span class="fs-6 fw-semibold" id="disp_per_day_salary">Rs 0.00</span>
                                    </div>
                                    <div class="col-12 col-md-3">
                                        <small class="text-muted d-block">Attendance Deductions</small>
                                        <span class="fs-6 fw-semibold text-danger" id="disp_attendance_deduction">Rs 0.00</span>
                                    </div>
                                    <div class="col-12 col-md-3">
                                        <small class="text-muted d-block">Net Calculated Salary</small>
                                        <span class="fs-5 fw-bold text-success" id="disp_net_salary">Rs 0.00</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-12 col-md-4">
                        ...
                        <input type="number" step="0.01" name="salary_amount" id="salary_amount" class="form-control @error('salary_amount') is-invalid @enderror" value="{{ old('salary_amount', 0) }}" required min="0">
                        @error('salary_amount')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const summaryContainer = document.getElementById('payroll_summary_container');
                    ...
                    try {
                        ...
                        if (res.success && res.data) {
                            ...
                            if (parseFloat(paidInput.value) === 0 || paidInput.value === '') {
                                paidInput.value = d.net_salary.toFixed(2);
                            }
                        ...
                        btnCalculate.innerHTML = '<i class="ti ti-calculator me-1"></i>Calculate Payroll';
                    }
                }

                btnCalculate.addEventListener('click', triggerCalculate);
                userSelect.addEventListener('change', triggerCalculate);
                periodInput.addEventListener('change', triggerCalculate);

                if (userSelect.value && periodInput.value) {
                    triggerCalculate();
                } else {
                    updateRemaining();
                }
