...
@section('content')
    ...
    <div class="d-flex align-items-center justify-content-between gap-2 mb-4 flex-wrap">
        ...
        <div class="d-flex align-items-center gap-2 flex-wrap">
            @can('labour-roles-create')
                <a href="{{ route('labour_roles.import.form') }}" class="btn btn-outline-primary">Import</a>
                <a href="{{ route('labour_roles.create') }}" class="btn btn-primary shadow-sm">
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-header bg-white border-bottom">
            <form action="{{ route('labour_roles.index') }}" method="GET" class="row g-3 align-items-end m-0">
                <div class="col-12 col-lg-4">
                    <label class="form-label">Search</label>
                    <div class="input-icon input-icon-start position-relative">
                        <span class="input-icon-addon text-dark"><i class="ti ti-search"></i></span>
                        <input type="text" name="q" class="form-control" placeholder="Search labour roles" value="{{ request('q') }}">
                    </div>
                </div>
                ...
                <div class="col-12 col-md-6 col-lg-2 d-flex gap-2">
                    ...
                    <a href="{{ route('labour_roles.index') }}" class="btn btn-outline-secondary w-100 shadow-sm">Reset</a>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm">
            ...
            <div class="table-responsive custom-table">
                <table class="table table-hover table-nowrap align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="text-center text-muted py-4">No labour roles found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        @if ($labourRoles->hasPages())
            <div class="card-footer bg-white d-flex justify-content-end">
                {{ $labourRoles->withQueryString()->links() }}
            </div>
        @endif
    </div>
@endsection
