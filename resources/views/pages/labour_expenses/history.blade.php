...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="mb-0">Labour Expense History</h4>
    </div>
    ...
    <div class="table-responsive">
        <table class="table table-bordered">
            ...
            <tbody>@forelse($transactions as $tx)<tr>
                ...
                <td>-</td>
                <td>-</td>
                <td class="d-flex gap-2"><a href="#" class="btn btn-sm btn-outline-primary disabled"
                        aria-disabled="true">Edit</a>
                    <form method="POST" action="{{ route('labour-expenses.delete-record') }}"
                        onsubmit="return confirm('Delete this labour expense?');">@csrf<input type="hidden" name="id"
                            value="{{ $tx->id }}" /><input type="hidden" name="delete_reason"
                            value="Deleted by checkbox" /><button type="submit"
                            class="btn btn-sm btn-outline-danger">Delete</button></form>
                </td>
            </tr>@empty<tr>
                    <td colspan="17" class="text-center">No records</td>
                </tr>@endforelse</tbody>
        </table>
    </div>
    {{ $transactions->links() }}
