...
@section('content')
    ...
    <form action="{{ route('roles.store') }}" method="POST" id="roleForm" class="row g-4">
        @csrf
        ...
        <div class="col-xl-4">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-header bg-white border-0 pb-0">
                    ...
                    <p class="text-muted fs-13 mb-0">Set the basic information for this role.</p>
                </div>
                <div class="card-body">
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" rows="5" name="description"
                            placeholder="Explain what this role can access">{{ old('description') }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="rounded-3 bg-light p-3">
                        <span class="badge badge-soft-primary mb-2">{{ $permissions->count() }} Permissions</span>
                        <p class="text-muted mb-0 fs-13">Choose the modules and actions this role should see in the app.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-xl-8">
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-white border-0 pb-0">
                    ...
                    <p class="text-muted fs-13 mb-0">Sidebar visibility and page access both come from these permissions.</p>
                </div>
                <div class="card-body">
                                        ...
                                        @foreach($modulePermissions as $permission)
                                            <div class="col-md-6">
                                                <label class="border rounded-3 p-3 d-flex gap-3 align-items-start w-100">
                                                    <input class="form-check-input mt-1 permission-checkbox" type="checkbox" name="permissions[]"
